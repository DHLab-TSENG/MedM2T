...
class StaticDataset(Dataset):
    def __init__(self, static, targets, ids):
        ...
        self.targets = torch.from_numpy(targets).type(torch.FloatTensor)
        self.ids = ids
...
class StaticLoader():
    def __init__(self):
        ...
        self.targets = self.icu24_patients["icustay_days"].values
        self.targets_df = pd.DataFrame({"targets":self.targets}, index=self.ids)
    ...
    def get_dataset(self, CICU=False):
        if CICU:
            return StaticDataset(self.data[self.CICU_idx], self.targets[self.CICU_idx], self.CICU_ids)
        return StaticDataset(self.data, self.targets, self.ids)
