...
class ECGLoader():
    def __init__(self, ids, targets, tw_pth_list = [20, 40, 60, 80]):
        ...
        #read record dict
        #key: ecg_id (last ecg), value: {"prev_ecg_id":[ecg_id], "days":[days]}
        #the last ecg is the index of the record
        self.record_dict_fneme = "ecg_dict.pkl"
        with open(data_path+self.record_dict_fneme, 'rb') as f:
            self.record_dict = pickle.load(f)

        self.valid_idx = np.where(np.isin(self.ids, list(self.record_dict.keys())))[0]
        self.valid_ids = self.ids[self.valid_idx]
        
        #ecg2record: ecg_id to record_id and target
        self.ecg2record = {}
        ecg_time_diff = []
        for id, ecgs in self.record_dict.items():
            for ecg_id in ecgs["prev_ecg_id"]:
                self.ecg2record[ecg_id] = {"record_id": id, "target": id2target[id]}
            self.record_dict[id]["target"] = id2target[id]
            ecg_time_diff.extend(ecgs["days"])
        ...
        self.ecg_ids = np.load(data_path+"ecg_signal_all_id.npy")
        self.ecg_targets = np.array([self.ecg2record[ecg_id]["target"] for ecg_id in self.ecg_ids])
        self.ecg_ids2idx = dict(zip(self.ecg_ids, range(len(self.ecg_ids)))) 

        #read signal, order by ecg_id
        # self.sig_fname = "24hr_icu_ecg.npy"
        self.sig_fname = "ecg_signal_all.npy"
        self.ecg_signal = np.load(data_path+self.sig_fname)
        self.sigs_shape = self.ecg_signal[0].shape
        self.ecg_signal_None = np.zeros(self.sigs_shape)
        
        #read statement, transform to tokens
        self.statement_fname = "ecg_statement_df.pkl"
        ecg_statement_df = pd.read_pickle(data_path+self.statement_fname)
        self.ecg_statement_cols = ecg_statement_df.columns
    ...
    def get_ecg_data(self, data_type, last=False, only_valid=False):
        ...
        if last:
            ...
            for id in ids:
                if id in self.record_dict:
                    last_ecg = self.record_dict[id]["prev_ecg_id"][-1]
    ...
    def get_bag_data(self, record_id):
        if record_id in self.record_dict:
            ecg_ids = self.record_dict[record_id]["prev_ecg_id"]
            ecg_wins = list(pd.cut(self.record_dict[record_id]["days"], 
    ...
    def get_recordi2ecgi(self, record_idx_list, only_valid = True):
        ids = self.valid_ids if only_valid else self.ids
        ecg_idx_list = []
        for record_idx in record_idx_list:
            id = ids[record_idx]
            ecg_idx_list.extend([self.ecg_ids2idx[ecg_id] for ecg_id in self.record_dict[id]["prev_ecg_id"]])
        return ecg_idx_list
